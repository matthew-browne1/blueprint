...
function generateChartsA(data) {
  ...
  data.forEach((entry) => {
    ...
    if (response_chartData[channelGroup].length < 40) {
      response_chartData[channelGroup].push({
        x: entry.Budget,
        y: entry["Predicted Revenue"],
      });
    }
  });
  ...
  const response_curve_chartData = {
    datasets: Object.keys(response_chartData).map((channelGroup) => {
      return {
        ...
        borderColor:
          "#" + (Math.random().toString(16) + "000000").substring(2, 8), // Random color for each line
        fill: false,
        radius: 0,
      };
    }),
  ...
  if (response_curve_chart === null) {
    response_curve_chart = new Chart(
      document.getElementById("response_curve_chart"),
      {
        type: "line",
...
function generateChartsB(data) {
  ...
  data.forEach((entry) => {
    ...
    budget_chartData.datasets[1].data.push(
      entry["Historical Profit (not optimised)"]
    );
    budget_chartData.datasets[2].data.push(
      entry["Historical Profit (optimised)"]
    );
  ...
  if (budget_curve_chart === null) {
    budget_curve_chart = new Chart(
      document.getElementById("budget_curve_chart"),
      {
        type: "line",
...
function generateChartsD(data) {
  ...
  if (budget_response_chart === null) {
    budget_response_chart = new Chart(
      document.getElementById("response_budget_chart"),
      {
        type: "line",
