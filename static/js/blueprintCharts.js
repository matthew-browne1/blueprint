...
$(document).ready(function () {
  ...
  function populateDropdown(selector, options) {
    ...
    dropdown.empty();
    var selectAllOption = $("<option></option>")
      .attr("value", "all")
      .text("Select All");
  ...
  chartsSocket
    .on("dropdown_options", function (data) {
  ...
  function enableCustomDropdown(selector) {
    ...
    dropdownOptions.on("click", "input[type='checkbox']", function () {
      ...
      var selectedOptions = dropdownOptions
...
function generateChartsA(data, metric) {
  ...
  const processedData = data.reduce((acc, entry) => {
    ...
    // Calculate ROIs
    acc[key].ST_ROI =
      acc[key].Budget !== 0 ? acc[key].ST_Revenue / acc[key].Budget : 0;
    acc[key].LT_ROI =
      acc[key].Budget !== 0 ? acc[key].LT_Revenue / acc[key].Budget : 0;
    acc[key].Total_ROI =
      acc[key].Budget !== 0
  ...
  const lt_revData = Object.values(processedData).map(
    (entry) => entry.LT_Revenue
  );
  const st_revData = Object.values(processedData).map(
    (entry) => entry.ST_Revenue
  );
  ...
  if (budget_scenario_chart === null) {
    budget_scenario_chart = new Chart(
      document.getElementById("budget_scenario_chart"),
      {
        type: "bar",
  ...
  if (revenue_scenario_chart === null) {
    revenue_scenario_chart = new Chart(
      document.getElementById("revenue_scenario_chart"),
      {
        type: "bar",
    ...
    revenue_scenario_chart.data.datasets[0].data =
      revenue_scenario_chartData.datasets[0].data;
    revenue_scenario_chart.data.datasets[1].data =
      revenue_scenario_chartData.datasets[1].data;
  ...
  if (roi_scenario_chart === null) {
    roi_scenario_chart = new Chart(
      document.getElementById("roi_scenario_chart"),
      {
        type: "bar",
    ...
    roi_scenario_chart.data.datasets[0].data =
      roi_scenario_chartData.datasets[0].data;
    roi_scenario_chart.data.datasets[1].data =
...
function generateChartsB(data, metric) {
  ...
  const processedDataPerChannel = data.reduce((acc, entry) => {
    ...
    // Calculate ROIs
    acc[scenarioKey].Channels[channelKey].ST_ROI =
      acc[scenarioKey].Channels[channelKey].Budget !== 0
        ? acc[scenarioKey].Channels[channelKey].ST_Revenue /
          acc[scenarioKey].Channels[channelKey].Budget
        : 0;
    acc[scenarioKey].Channels[channelKey].LT_ROI =
      acc[scenarioKey].Channels[channelKey].Budget !== 0
        ? acc[scenarioKey].Channels[channelKey].LT_Revenue /
          acc[scenarioKey].Channels[channelKey].Budget
        : 0;
    acc[scenarioKey].Channels[channelKey].Total_ROI =
      acc[scenarioKey].Channels[channelKey].Budget !== 0
        ? (acc[scenarioKey].Channels[channelKey].ST_Revenue +
            acc[scenarioKey].Channels[channelKey].LT_Revenue) /
          acc[scenarioKey].Channels[channelKey].Budget
  ...
  const channel_budget_data = scenarios.map((scenario, index) => {
    const data = channels.map((channel) => {
      const budget =
        processedDataPerChannel[scenario]?.Channels[channel]?.Budget || 0; // Handling potential undefined values
      return budget;
    });
    return {
      label: scenario,
  ...
  const channel_revenue_data = scenarios.map((scenario, index) => {
    const data = channels.map((channel) => {
      const stRevenue =
        processedDataPerChannel[scenario]?.Channels[channel]?.ST_Revenue || 0;
      const ltRevenue =
        processedDataPerChannel[scenario]?.Channels[channel]?.LT_Revenue || 0;
  ...
  const channel_ROI_data = scenarios.map((scenario, index) => {
    const data = channels.map((channel) => {
      const totalROI =
        processedDataPerChannel[scenario]?.Channels[channel]?.Total_ROI || 0;
      return totalROI;
    });
    return {
      label: scenario,
...
function generateChartsD(data, metric) {
  ...
  const laydown_channel_data = Object.keys(
