...
const processedDataLaydown = data.reduce((acc, entry) => {
    const key = entry.Scenario;
    const timePeriodParts = entry.Time_Period.split('/');
    const timePeriod = new Date(`${timePeriodParts[1]}/${timePeriodParts[0]}/${timePeriodParts[2]}`);
    const monthYear = timePeriod.toLocaleString('default', { month: 'short', year: 'numeric' });
...
const selectedScenario = "Actual Budget - Optimised"; // Initial scenario selection
