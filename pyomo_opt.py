...
class Optimiser:
    ...
    def beta_opt(channel_input, laydown, num_weeks=1000):
        ...
        for stream in streams:
            def roi_expr(model):
                return calculated_roi_per_stream(stream, model.beta[stream])
            model.roi_constraint.add(expr=roi_expr(model) == current_roi_dict[stream])

        model.obj = Objective(expr=sum((calculated_roi_per_stream(stream, model.beta[stream]) - current_roi_dict[stream]) ** 2 for stream in streams), sense=minimize)
        ...
        if results.solver.termination_condition == TerminationCondition.optimal:
            ...
            print(f"manual betas = {manual_betas}")
            print(f"optimised betas = {optimal_beta_values}")
